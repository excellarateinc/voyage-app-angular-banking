...
import { SendMoney } from './send-money/send-money.model';
...
export class AccountsService {
  ...
  sendMoney(model: SendMoney): Observable<any> {
    return this.http.post(`${environment.API_URL}/banking/accounts/transfers/external`, model)
      .map(response => response)
      .catch(error => Observable.throw(error.json()));
  }
}
